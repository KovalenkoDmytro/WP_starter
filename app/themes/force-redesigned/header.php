...
<body <?php body_class(); ?>>
    <header
        ...
        <div class="justify-content-between container-fluid">
            ...
            if ( has_custom_logo() ) {
                echo '<a class="navbar-brand" href="' . esc_url( home_url( '/' ) ) . '" rel="home"><img width="107" src="' . esc_url( $logo[0] ) . '" alt="' . get_bloginfo( 'name' ) . '"></a>';
            } else {
                echo '<a class="navbar-brand" href="' . esc_url( home_url( '/' ) ) . '" rel="home"><img width="107" src="' . esc_url( get_template_directory_uri() ) . '/assets/images/logo.svg" alt="' . get_bloginfo( 'name' ) . '"></a>';
            }
            ?>
...
if ( $post_id == '16' || $post_id == '20' ) {
    ...
    if ( $thumbnail_id ) {
        ...
        if ( $image_src ) {
            $backimage = $image_src[0];
        }
    }
    ?>

    <div class="InnerBanner-bg" style="background-image: url(<?php echo esc_url( $backimage ); ?>);">&nbsp;
    </div>
    <?php
} ?>